...
class MoEFFN(nn.Module):
    """
    MoE Feed-Forward Network for MNIST
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % 100 == 0:
            print(
                ...
                f"Balance: {load_balance_loss.item():.4f}) Acc: {100.0 * correct / total:.2f}%"
            )
...
def train_moe(num_experts=4, top_k=2, epochs=5, batch_size=64, lr=0.001, alpha=0.01):
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f"  Train Loss: {train_loss:.4f} (Cls: {cls_loss:.4f}, Balance: {balance_loss:.4f})"
        )
