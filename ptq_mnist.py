...
class QuantizationCalibrator:
    ...
    def collect_stats(self, data_loader, num_batches=10):
        ...
        def get_hook(name):
            def hook(module, input, output):
                ...
                self.activation_stats[name]["min"] = min(
                    self.activation_stats[name]["min"], output.min().item()
                )
                self.activation_stats[name]["max"] = max(
                    self.activation_stats[name]["max"], output.max().item()
                )
        ...
        print(
            f"Calibration complete. Collected stats for {len(self.activation_stats)} layers."
        )
...
class QuantizedLinear(nn.Module):
    ...
    def __init__(self, in_features, out_features):
        ...
        self.register_buffer(
            "q_weight", torch.zeros(out_features, in_features, dtype=torch.uint8)
        )
    ...
    def from_float(float_linear, num_bits=8):
        ...
        # Quantize weights
        q_weight, scale, zero_point = quantize_tensor(
            float_linear.weight.data, num_bits
        )
...
def apply_ptq_to_model(model, calibration_loader, device, is_moe=False, num_bits=8):
    ...
    _, quant_acc, quant_time = evaluate(
        quantized_model, device, calibration_loader, is_moe
    )
    ...
    print("=" * 60)
    print(
        f"Size Reduction: {orig_size:.2f} MB → {quant_size:.2f} MB "
        f"({(1 - quant_size / orig_size) * 100:.1f}% reduction)"
    )
    print(
        f"Accuracy: {orig_acc:.2f}% → {quant_acc:.2f}% "
        f"({quant_acc - orig_acc:+.2f}% change)"
    )
    print(
        f"Inference Time: {orig_time:.2f}s → {quant_time:.2f}s "
